...
bot.on("message", (msg) => {
  ...
  if (msg.chat.type === "private") {
    switch (command) {
      case "/start":
        const startMessage = `ЧТОБЫ ИЗБЕЖАТЬ МОШЕННИЧЕСТВА И НЕЗАКОННЫЕ ТЕМЫ, МЫ ЗАПУСТИЛИ ПЛАТНУЮ РЕКЛАМУ.БОТ ОПЛАТЫ, ПРЕДОСТАВЛЯЮЩИЙ ДОСТУП К РАЗМЕЩЕНИЮ РЕКЛАМЫ В НАШЕМ ЧАТЕ.СТОИМОСТЬ РЕКЛАМЫ:30 ДНЕЙ - 500 ₽ПОСЛЕ ОПЛАТЫ РЕКЛАМЫ, ВЫ МОЖЕТЕ В СВОБОДНОМ ДОСТУПЕ ПУБЛИКОВАТЬ СВОИ ПОСТЫ.
        `;
